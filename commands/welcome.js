...
const DEFAULT_GIFS = [
    'https://i.imgur.com/BglAjSy.gif', 'https://i.imgur.com/5iR2Kdg.gif', 
...
module.exports = {
    ...
    async handleWelcome(member, SETTINGS_FILE) {
        try {
            // 🚀 BACA DARI MESIN RAM (Anti-Lag saat banyak member join bersamaan)
            const guildData = member.client.checkDatabase(member.guild.id);
            ...
            const welcomeTexts = [
                ...
                `The scent of blood 🩸 and volcanic ash fills the air as you awaken in this savage land, <@${member.id}>. Welcome to prehistoric hell! There is no place for the weak here; your every step is watched by hungry, ancient eyes from the shadows of the undergrowth. Will you become a feared apex predator, or just another pile of bones ☠️ crushed beneath the weight of prehistoric giants? Rise, and fight for your life!`,
    ...
    async manageWcm(ctx, args, type, SETTINGS_FILE, settings, isRealAdmin, isCustomAdmin) {
        ...
        if (sub === 'gif') {
            ...
            fs.writeFile(SETTINGS_FILE, JSON.stringify(settings, null, 2), (err) => {
                if (err) console.error("Gagal save gif WCM:", err);
            });
        ...
        if (sub === 'rmv') {
            ...
            // 🚀 Simpan asinkron di latar belakang
            fs.writeFile(SETTINGS_FILE, JSON.stringify(settings, null, 2), (err) => {
                if (err) console.error("Gagal remove gif WCM:", err);
            });
